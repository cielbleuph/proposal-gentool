<?php
    ...
    $methodologiesPage->addText('The investigation uses all available resources, software or data files available on the internet to find out if the target assets are published online.', null, $defaultParagraphStyle);
    ...
    $methodologiesPage->addText("Cyber Threat Intelligence (CTI) can still be described as a nascent and fast-developing field. However, the practice of intelligence itself is historically and commercially a very well-established discipline.", null, $defaultParagraphStyle);

    $methodologiesPage->addText("Using an intelligence-led approach has long been accepted as best practice in the realm of conventional security. Without it, organisations will invariably defend against too little, because they don’t understand the threats they face, or try to defend against all potential threats–an unsustainable approach that may also impair the organisation’s ability to operate effectively.", null, $defaultParagraphStyle);

    $methodologiesPage->addText("For example, accompany looking to build a facility in a potentially hostile environment would first seek intelligence on the threat posed by malicious actors in the vicinity before trying to adopt appropriate security controls.", null, $defaultParagraphStyle);

    $methodologiesPage->addText("The Threat Intelligence methodology contains the following phases:", null, $defaultParagraphStyle);

    $methodologiesPage->addTextBreak();

    $methodologiesPage->addText("Environmental Context", array("bold"=>true), $defaultParagraphStyle);
    $methodologiesPage->addText("A deep understanding of your organization, including your organization’s entire attack surface; threats, risks, and opportunities targeting your organization and industry; and your organization’s internal and external network and operations. Gaining this understanding is a continuous process and influences what data is needed to perform cyber intelligence.", null, $defaultParagraphStyle);

    $methodologiesPage->addText("Data Gathering", array("bold"=>true), $defaultParagraphStyle);
    $methodologiesPage->addText("Through automated and labor-intensive means, data and information is collected from multiple internal and external sources for analysts to analyze to answer organizational intelligence requirements.", null, $defaultParagraphStyle);

    $methodologiesPage->addText("Threat Analysis", array("bold"=>true), $defaultParagraphStyle);
    $methodologiesPage->addText("Assessing technical telemetry and non-technical data pertaining to specific threats to your organization and industry to inform cybersecurity operations/actions and Strategic Analysis. Threat Analysis is built on operational and tactical analysis and enhances CSO/CISO and other mid- to senior level decision making.", null, $defaultParagraphStyle);

    $methodologiesPage->addText("Strategic Analysis", array("bold"=>true), $defaultParagraphStyle);
    $methodologiesPage->addText("Holistically assessing threats, risks and opportunities to enhance executive decision-making pertaining to organization-wide vital interests such as financial health, brand, stature, and reputation.", null, $defaultParagraphStyle);

    $methodologiesPage->addText("Reporting and Feedback", array("bold"=>true), $defaultParagraphStyle);
    $methodologiesPage->addText("Communication between analysts and decision makers, peers, and other intelligence consumers regarding their products and work performance. Reporting and feedback help identify intelligence requirements and intelligence gaps.", null, $defaultParagraphStyle);

    $methodologiesPage->addTextBreak();

    $methodologiesPage->addText('The Different Levels of Cyber Threat Intelligence', array("bold"=>true, "size" => 13), $defaultParagraphStyle );
    $methodologiesPage->addText("As with conventional intelligence, there are different levels of cyber threat intelligence: operational, tactical, and strategic. Each level differs in the nature and format of the material conveyed, its intended audience and its application. These are summarised:", null, $defaultParagraphStyle);

    $dark1 = $methodologiesPage->addListItemRun(0);
    ...
    $dark2 = $methodologiesPage->addListItemRun(0);
    ...
    $dark3 = $methodologiesPage->addListItemRun(0);
    ...
    $dark4 = $methodologiesPage->addListItemRun(0);
    ...
    $dark5 = $methodologiesPage->addListItemRun(1);
    ...
    $dark6 = $methodologiesPage->addListItemRun(1);
    ...
    $dark7 = $methodologiesPage->addListItemRun(1);
    ...
    $dark8 = $methodologiesPage->addListItemRun(1);
    ...
    $dark10 = $methodologiesPage->addListItemRun(0);
    ...
    $dark11 = $methodologiesPage->addListItemRun(0);
    ...
    $dark12 = $methodologiesPage->addListItemRun(0);
    ...
    $dark13 = $methodologiesPage->addListItemRun(0);
    ...
    $dark14 = $methodologiesPage->addListItemRun(0);
    ...
    $dark15 = $methodologiesPage->addListItemRun(0);
